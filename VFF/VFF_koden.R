pacman::p_load(tidyverse, tidymodels, caret, rvest, rlist, 
               ...
               DBI, RSQLite, dbplyr, ggplot2, MASS, ISLR2, boot,
               viridis, pls, repurrrsive)

# Indlæsning og rens af guld.xlsx -----------------------------------------

# Indlæs data
guld_data <- read_xlsx("data/Guld.xlsx", na = "")

# Konverter kolonnenavne til småt
colnames(guld_data) <- tolower(colnames(guld_data))

# Databehandling i en enkelt pipeline
guld_data_final <- guld_data |> 
  # Håndter datoformat
  mutate(dato = case_when(
    !is.na(as.numeric(dato)) ~ as.Date(as.numeric(dato), origin = "1899-12-30"),
    TRUE ~ as.Date(dato, format = "%d.%m.%Y")
  )) |> 
  # Filtrér relevante rækker
  filter(!is.na(guld_menu_stk), !is.na(antal_bestilte), dato >= as.Date("2021-07-31")) |> 
  # Fjern unødvendig kolonne
  dplyr::select(-gule_poletter_stk) |> 
  # Konverter kategoriske variabler og ret fejl
  mutate(
    kamp = as.factor(if_else(kamp == "SDR", "SJF", kamp)),
    # Tilføj kamp_type
    kamp_type = case_when(
...
# Inspektion af den rensede data
glimpse(guld_data_final)
colSums(is.na(guld_data_final))

# Nedenstående frem til linje 309 er udkonverteret. Der er vedhæftet Rds filer i mappen "data" eller kan du markere det kommenterede cmd/ctrl + shift + c for at køre koden. OPS - husk at indsætte din egen API-key til DMI 

# # Webscraping og rens af SuperStats.dk ------------------------------------
# 
...
#   udvalgte_tabeller <- alle_tabeller[1:(length(alle_tabeller))] # er egentlig lige meget
#   
...
# superstats_data
# 
# # Bearbejd superstats data i én samlet pipeline
...
# 
# # Inspektion af data
# glimpse(superstats_dataframe)
# head(superstats_dataframe)
# 
# 
# # Indhentning af DST med API samt rensning --------------------------------
# 
...
#   pull(1) |>  #tager kolonneoverskrift efte $
#   mutate(
...
# # Hent DMI fra API --------------------------------------------------------
# 
# # DMI API parameters - genbruges
# base_dmi <- "https://dmigw.govcloud.dk/v2/"
# info_dmi <- "metObs/collections/observation/items?"
# api_key <- "indsæt din API Key her"
# 
# # Generisk funktion til at hente DMI data
# get_dmi_data <- function(parameter_id, date_param, limit, column_name) {
#   # Build URL
#   query_params <- paste0("stationId=06060&parameterId=", parameter_id, "&datetime=", date_param, "&limit=", limit)
#   full_url <- paste0(base_dmi, info_dmi, query_params, api_key)
#   
#   # API call
...
#     stop("API error: Status ", api_response$status_code)
#   }
#   
#   # Process response
#   api_char <- base::rawToChar(api_response$content)
#   api_json <- jsonlite::fromJSON(api_char, flatten = TRUE)
#   list_data <- as.list(api_json)
#   cold_data <- as.data.frame(do.call(cbind, list_data))
#   
#   # Tager relevante kolonner
#   col1 <- as.vector(cold_data[7])  # observation timestamp
#   col3 <- as.vector(cold_data[10]) # measurement value
#   add_cols <- c(col1, col3)
#   
#   # Lav Tibble med dynamisk navngivning
#   tibble_data <- as_tibble(add_cols) %>%
#     rename(
#       "observationstidspunkt" = "features.properties.observed",
#       !!column_name := "features.properties.value"
#     )
#   
#   return(tibble_data)
# }
# 
# # Alle sæsoner på vindhastighed
# wind_speed_21_22 <- get_dmi_data("wind_speed_past1h", "2021-07-31T00:00:00Z/2022-05-15T23:59:00Z", 6928, "vindhastighed")
# wind_speed_22_23 <- get_dmi_data("wind_speed_past1h", "2022-07-17T00:00:00Z/2023-05-29T23:59:00Z", 7600, "vindhastighed")
# wind_speed_23_24 <- get_dmi_data("wind_speed_past1h", "2023-07-28T00:00:00Z/2024-05-25T23:59:00Z", 43588, "vindhastighed")
# 
# # Alle sæsoner på nedbør pr. time
# precip_21_22 <- get_dmi_data("precip_dur_past1h", "2021-07-31T00:00:00Z/2022-05-15T23:59:00Z", 8000, "nedbør")
# precip_22_23 <- get_dmi_data("precip_dur_past1h", "2022-07-17T00:00:00Z/2023-05-29T23:59:00Z", 7600, "nedbør")
# precip_23_24 <- get_dmi_data("precip_dur_past1h", "2023-07-28T00:00:00Z/2024-05-25T23:59:00Z", 7264, "nedbør")
# 
# # Alle sæsoner på temperaturer pr. time
# temperatur_21_22 <- get_dmi_data("temp_mean_past1h", "2021-07-31T00:00:00Z/2022-05-15T23:59:00Z", 6930, "temperatur")
# temperatur_22_23 <- get_dmi_data("temp_mean_past1h", "2022-07-17T00:00:00Z/2023-05-29T23:59:00Z", 45630, "temperatur")
# temperatur_23_24 <- get_dmi_data("temp_mean_past1h", "2023-07-28T00:00:00Z/2024-05-25T23:59:00Z", 7266, "temperatur")
# 
# 
# # Join api ----------------------------------------------------------------
# 
# precip_alle <- bind_rows(precip_21_22,
#                          precip_22_23,
#                          precip_23_24)
# 
# #Bind af wind
# wind_alle <- bind_rows(wind_speed_21_22,
#                        wind_speed_22_23,
#                        wind_speed_23_24)
# 
# #Bind af Temp
# temp_alle <- bind_rows(temperatur_21_22,
#                        temperatur_22_23,
#                        temperatur_23_24)
# 
# dmi_join <- precip_alle |> 
#   left_join(wind_alle, join_by("observationstidspunkt" == "observationstidspunkt")) |> 
#   left_join(temp_alle, join_by("observationstidspunkt" == "observationstidspunkt"))
# 
# # Rens af dmi api ---------------------------------------------------------
# 
# dmi_rens <- dmi_join |>
#   mutate(observationstidspunkt = as_datetime(observationstidspunkt, format = "%Y-%m-%dT%H:%M:%SZ", tz = "UTC")) |> 
#   filter(format(observationstidspunkt, "%H:%M:%S") >= "09:00:00" & format(observationstidspunkt, "%H:%M:%S") <= "21:00:00") |>
#   separate(observationstidspunkt, into = c("dato", "tidspunkt"), sep = " ") |> 
#   mutate(dato = as.Date(dato),
#     tidspunkt = as.factor(tidspunkt)
#   )
# 
# 
# #koden filtrer så det kun er datoer fra kampdage
# dmi_filter <- dmi_rens |>
#   filter(dato %in% guld_data_final$dato)
# 
# #Gennemsnit af variabler for kun at have en observation pr. dag
# dmi_gns <- dmi_filter |>   
#   group_by(dato) |> 
#   mutate(nedbør = mean(nedbør),
#          temperatur = mean(temperatur),
#          vindhastighed = mean(vindhastighed)) |> 
#   filter(tidspunkt == "14:00:00")
# 
# dmi_final <- dmi_gns |> 
#   dplyr::select(-tidspunkt)


# Gem datasæt som rds, så det ikke skal køres hver gang -------------------
# saveRDS(superstats_dataframe, file = "data/superstats_final.rds")

#Load RDS
superstats_data1 <- readRDS("data/superstats_final.rds")
...
join_r <- guld_data_final |>
  left_join(superstats_data1, join_by("dato" == "dato")) |>
...
join_final <- join_all |> 
  ...
  dplyr::select(-år, -måned, -dag, -kamp_type.y, -kamp_type.x, -kamp, -antal_max, -guld_menu_stk, -udehold, -hjemmehold, -hjemmescore, -udescore, -tilskuere, -nedbør) |> 
...
bestlam1 <- ridge_cv.out$lambda.min
# Finde RMSE på træning ridge cross-validation med den bedste lambda
rmse_ridge_cv <- sqrt(ridge_cv.out$cvm[ridge_cv.out$lambda == bestlam1])
...
dim(data_train)[2]  # Der er 11 variabler og dermed 10 prædiktorer
...
for (j in 1:k) { # her gennemløbes alle folds
  ...
  for (i in 1:11) { # her gennemløbes alle kandidatmodeller
    ...
    cv.errors[j, i] <-
      mean((data_train$antal_bestilte[folds == j] - pred)^2) # Her udregnes MSE for hver fold for hver kandidatmodel 
  }
}

mean.cv.errors <- apply(cv.errors, 2, mean) # apply er en smart funktion, der 
# gennemløber alle rækker og tager gennemsnittet henover hver søjle, som svarer til hver kandidatmodel. # Kan også bruge en map funktion
...
which.min(mean.cv.errors) #Finder den modelstørrelse med lavest MSE

# Her fittes modellen til ALLE træningsdata ud folds
reg.best <- regsubsets(antal_bestilte ~ ., data = data_train,
                       nvmax = (dim(data_train)[2]-1))
coef(reg.best, which.min(mean.cv.errors)) # # Henter koefficienter for den bedste model

pred_best_subset <- predict(reg.best, data_test, id = which.min(mean.cv.errors)) # og her 1
...
# Prædiktion af den bedste model ------------------------------------------
